...
BarWidget {
  ...
  function formatted(date) {
    ...
    var text = Qt.formatDateTime(date, fmt)
    // Qt only uses a 12-hour clock (hour 1-12) when an AM/PM marker
    // ("AP"/"ap") is present, and that marker always prints the AM/PM text.
    // For a format with a lowercase-hour but no marker (e.g. "h:mm"), render
    // the 12-hour form and strip the suffix so "5:30" appears instead of
    // "17:30" or "5:30 PM".
    if (fmt.indexOf("h") !== -1 && !/AP|ap/.test(fmt)) {
      text = Qt.formatDateTime(date, fmt + " AP")
      text = text.replace(/\s*\b(AM|PM)\b\s*$/i, "")
    }
    return text
  }
